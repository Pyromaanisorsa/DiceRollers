...
[CreateAssetMenu(fileName = "StationaryMeleeFlow", menuName = "CombatData/AbilityFlow/StationaryMelee")]
public class StationaryMeleeFlow : AbilityFlow
{
    ...
    public override void Execute(CombatParticipant caster, AbilityData data)
    {
        ...
        // Get tiles in cardinal Melee range
        List<GridTile> tiles = GridManager.Instance.GetTilesInRange(caster.gridPosition, 1, data.ignoreStartSelection, data.throughObstacles);
    ...
    public override Vector2Int CalculateMovePositionAI(CombatParticipant caster, AbilityData data)
    {
        ...
        foreach (GridTile moveTile in moveTiles)
        {
            ...
            List<GridTile> targetTiles = GridManager.Instance.GetTilesInRange(moveTile.GridPosition, 1, data.ignoreStartSelection, data.throughObstacles);
