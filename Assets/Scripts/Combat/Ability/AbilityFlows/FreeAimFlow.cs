...
// Flow that can be targeteted on any tile in ability range
[CreateAssetMenu(fileName = "FreeAimFlow", menuName = "CombatData/AbilityFlow/FreeAim")]
public class FreeAimFlow : AbilityFlow
{
    ...
    public override void Execute(CombatParticipant caster, AbilityData data)
    {
        ...
        // Show all reachable tiles in ability range
        List<GridTile> tiles = GridManager.Instance.GetTilesInRange(caster.gridPosition, data.abilityStats.Range, data.ignoreStartSelection, data.throughObstacles);
    ...
    public override List<Vector2Int> ExecuteAI(CombatParticipant caster, AbilityData data)
    {
        ...
        List<GridTile> tiles = GridManager.Instance.GetTilesInRange(casterPosition, data.abilityStats.Range, data.ignoreStartSelection, data.throughObstacles);
    ...
    public override Vector2Int CalculateMovePositionAI(CombatParticipant caster, AbilityData data)
    {
        ...
        foreach (GridTile moveTile in moveTiles)
        {
            ...
            List<GridTile> targetTiles = GridManager.Instance.GetTilesInRange(moveTile.GridPosition, data.abilityStats.Range, data.ignoreStartSelection, data.throughObstacles);
